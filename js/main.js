...
const sco = {
  ...
  addPhotoFigcaption() {
    ...
    document
      ...
      .forEach((image) => {
        ...
        if (next && next.classList && next.classList.contains("img-alt")) return;
        ...
        if (captionText) {
          ...
          const captionEl = image.nextElementSibling;

          if (captionEl && typeof renderMathInElement === "function") {

            renderMathInElement(captionEl, {
  ...
  aboutPersonalityChromaKey() {

    if (!PAGE_CONFIG || PAGE_CONFIG.page !== "about") return;

    const img = document.getElementById("about-personality-figure");

    if (!img || img.dataset?.chromaKeyed === "true") return;
    ...
    const run = () => {
      ...
      try {
        ...
        for (let p = 0, i = 0; p < alpha.length; p++, i += 4) {
          ...
          else if (dist >= max) k = 1;

          else k = smoothstep((dist - hard) / soft);



          alpha[p] = Math.round(255 * a0 * k);

        }



        // light alpha blur (3x3 box) to reduce jagged edges
        ...
        // apply blurred alpha + defringe (remove background color bleeding on edges)

        for (let p = 0, i = 0; p < blurred.length; p++, i += 4) {

          const a = blurred[p] / 255;
          ...
          if (a < 1) {
            ...
            data[i + 1] = clamp255((data[i + 1] - bg * (1 - a)) / a);
...
window.refreshFn = () => {
  ...
  if (page === "music" && typeof initializeMusicPlayer === "function") {
